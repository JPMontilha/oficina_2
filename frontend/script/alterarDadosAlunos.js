document.addEventListener("DOMContentLoaded", function () {
  ...
  async function alterarDadosAluno(novoEmail, novaSenha) {
    try {
      ...
      // Se há novo email, atualiza apenas o email (não o objeto user inteiro)
      if (novoEmail && novoEmail !== userData.email) {
        dadosAtualizacao["user.email"] = novoEmail;
      }
      ...
      const alunoAtualizado = await alunosAPI.atualizar(
        aluno._id,
        dadosAtualizacao
      );
      ...
      try {
        ...
        if (error.message) {
          if (error.message.includes("não encontrado")) {
            ...
            errorMessage =
              "Você não tem permissão para alterar estes dados. Verifique com o administrador.";
